...
$sql = "UPDATE tb_not_mesa SET ic_notificacao = true WHERE cd_usuario = $amigoid and cd_mesa = $mesa;";
$conexao->query($sql);
        ...
        on m.cd_mesa = o.cd_mesa
          join tb_perfil as p 
            on o.cd_usuario = p.cd_usuario
              where p.cd_usuario = $meuid and n.ic_notificacao = 0";
...
if($resultnotificacaoMesa->num_rows>0) 
{
  while($row = $resultnotificacaoMesa->fetch_assoc()) 
  {
    ...
    while($row2 = $ResultNickRemetente->fetch_assoc()) 
    {
      ...
      $sqlMesa = "select nm_mesa from tb_mesa where cd_admin = $meuid and cd_mesa = $mesa";
