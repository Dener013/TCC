...
$sqlNoticia = "select cd_noticia, nm_noticia, img_noticia, date_format(dt_noticia, '%d-%m-%Y') as 'dt_noticia'
          from tb_noticia;";

$noticia = $conexao->query($sqlNoticia);
$carousel = $conexao->query($sqlNoticia);

// ------------------------Rank---------------------------- //

$sqlRanking = "select r.cd_ranking, p.nm_nick, e.sg_estado
...
$resultRanking = $conexao->query($sqlRanking);
...
<script type="text/javascript">
  function solitMesa(eu, ele, mesa){
    var meuid = eu;
    var amigoid = ele; 
    var mesa = mesa;
    var url = 'php/ajax_mesa.php?meuid='+meuid+'&amigoid='+amigoid+'&mesa='+mesa;
        $.get(url, function(dataReturn) {
          $('#mensagem-aja').html(dataReturn);
        });
  }
  function exluiMesa(eu, ele, mesa){
    var meuid = eu;
    var amigoid = ele; 
    var mesa = mesa;
    var url = 'php/ajax_desmesa.php?meuid='+meuid+'&amigoid='+amigoid+'&mesa='+mesa;
        $.get(url, function(dataReturn) {
          $('#mensagem-aja').html(dataReturn);
        });
  }
...
{
  while ($row2 = $noticia->fetch_assoc()) 
  {
    $id = $row2['cd_noticia'];
